...
sim/mult_tb: $(SRCS)
	mkdir -p sim
	cd sim && vcs $(SRCS) $(VCS_FLAGS) -msg_config=../vcs_warn.config -l compile.log -top mult_tb -o mult_tb
	bash check_compile_error.sh

sim/div_tb: $(SRCS)
	mkdir -p sim
	cd sim && vcs $(SRCS) $(VCS_FLAGS) -msg_config=../vcs_warn.config -l compile.log -top div_tb -o div_tb
	bash check_compile_error.sh
...
.PHONY: run_mult_tb
run_mult_tb: sim/mult_tb
	rm -f sim/dump.fsdb
	cd sim && ./mult_tb -l simulation.log
	bash check_sim_error.sh

.PHONY: run_div_tb
run_div_tb: sim/div_tb
	rm -f sim/dump.fsdb
	cd sim && ./div_tb -l simulation.log
	bash check_sim_error.sh
