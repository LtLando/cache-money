    ...
    Operation.LOAD: {
        "variants": [
            {   
                ...
                "formats": [
                    "lw {sink}, %pcrel_lo({offset})({base_reg})",
                ]
            },
            {
                "targets": MEM_HWORD_TARGETS,
                "formats": [
                    "lh {sink}, %pcrel_lo({offset})({base_reg})",
                    "lhu {sink}, %pcrel_lo({offset})({base_reg})",
                ]
            },
            {
                "targets": MEM_BYTE_TARGETS,
                "formats": [
                    "lb {sink}, %pcrel_lo({offset})({base_reg})",
                    "lbu {sink}, %pcrel_lo({offset})({base_reg})",
                ]
            }
        ]
    },
    Operation.STORE: {
        "variants": [
            {   
                "targets": MEM_WORD_TARGETS,
                "formats": [
                    "sw {sink}, %pcrel_lo({offset})({base_reg})",
                ]
            },
            {
                "targets": MEM_HWORD_TARGETS,
                "formats": [
                    "sh {sink}, %pcrel_lo({offset})({base_reg})",
                ]
            },
            {
                "targets": MEM_BYTE_TARGETS,
                "formats": [
                    "sb {sink}, %pcrel_lo({offset})({base_reg})",
                ]
            }
        ]
    }
...
class RV32IGenerator:
    ...
    def __update_sinks(self, new_sink: int):
        updated = []
        need_new = True

        for sink, c_rem in self.prev_sinks:
            if new_sink == sink:
                updated.append((sink, 5))
                need_new = False
            elif c_rem != 0:
                updated.append((sink, c_rem-1))

        if need_new:
            updated.append((new_sink, 5))

        self.prev_sinks = updated

    def __get_avail_sources(self) -> list[int]:
    ...
    def emit(self) -> str:
        if self.no_mem:
            op = choice(list(set(Operation).difference(set([Operation.LOAD]))))
        else:
        ...
        match op:
            ...
            case Operation.LOAD:
                ...
                offset_label = f"OFFSET_LABEL{self.mem_counter}"

                self.mem_counter += 1

                return f"{offset_label}: auipc x{base_reg}, %pcrel_hi({mem_target})\nnop\nnop\nnop\n" + \
                        inst_format.format(sink=f"x{new_sink}", offset=offset_label, base_reg=f"x{base_reg}")
