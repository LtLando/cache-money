...
OPS_MAP = {
    "reg_reg":  Operation.REG_REG,
    "reg_imm":  Operation.REG_IMM,
    "imm":      Operation.IMM,
    "load":     Operation.LOAD,
...
OP_INFO = {
    ...
    Operation.MULT: {
        "formats": [
            "mul {sink}, {src1}, {src2}",
            "mulh {sink}, {src1}, {src2}",
            "mulhu {sink}, {src1}, {src2}",
            "mulhsu {sink}, {src1}, {src2}"
        ]
    },
    Operation.REG_REG: {
        "formats": [
...
class RV32IGenerator:
    def __init__(self, spike: bool = False, no_mem: bool = False,
                 no_div: bool = False, no_mult: bool = False, only: str = None):
        self.prev_sinks = []
        self.spike = spike
        self.mem_counter = 0
        self.no_mem = no_mem
    ...
    def emit(self) -> str:
        ...
        if self.only is not None:
            if self.only not in OPS_MAP:
                raise ValueError(f"Operation type '{self.only}' not available")
            op = OPS_MAP[self.only]

        else:
            if self.no_mem:
                ops = ops.difference(set([Operation.LOAD, Operation.STORE]))
            if self.no_div:
                ops = ops.difference(set([Operation.DIV]))
            if self.no_mult:
                ops = ops.difference(set(Operation.MULT))
            op = choice(list(ops))
        ...
        match op:
            ...
            case Operation.DIV:
                ...
                inst_format = choice(OP_INFO[Operation.DIV]["formats"])

                return f"li x{src1}, {randint(-2**31-1, 2**32-1)}\nli x{src2}, {randint(-2**31-1, 2**32-1)}\n" + \
                    inst_format.format(sink=f"x{new_sink}", src1=f"x{src1}", src2=f"x{src2}")
            ...
            case Operation.MULT:
                ...
                inst_format = choice(OP_INFO[Operation.MULT]["formats"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-div", action="store_true", help="Do not generate divide instructions", default=False)
    parser.add_argument("--no-mult", action="store_true", help="Do not generate multiply instructions", default=False)
    parser.add_argument("--only", type=str, help=f"Only generate this type of instruction, types are {list(OPS_MAP.keys())}", default=None)
    args = parser.parse_args()

    gen = RV32IGenerator(spike=args.spike, no_mem=args.no_mem, no_div=args.no_div, no_mult=args.no_mult, only=args.only)
